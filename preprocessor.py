...
class Preprocessor5:
    ...
    def fit(self, df):
        ...
        if self.target_column in df.columns:
            ...
            first_group = df[df.iloc[:, 0] == 0]
            second_group = df[df.iloc[:, 0] == 1]

            first_mean = first_group.mean()
            second_mean = second_group.mean()

            self.column_means = (first_mean + second_mean) / 2
        else:
    ...
    def transform(self, df, poly=True):
        # Fill NaN values with column means
